...
## This project (Contract Loss Expert)

| Setting | Value |
|---------|--------|
| **`BRAND_NAME`** | `ContractLossExpert` |
| **`domain`** (when `NEXT_PUBLIC_SITE_URL=https://www.contractlossexpert.com`) | `contractlossexpert.com` |
| Contact webhook | **`/api/submit-lead`** primary → five-key JSON including **`domain`** |
| Google Sheets | Optional soft-fail — one shared tab + **Form Type** (Contact / Instruct) |
